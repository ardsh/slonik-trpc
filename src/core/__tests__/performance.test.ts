...
it("Post-processing without promises", async () => {
    const loader = makeQueryLoader({
        query,
        db,
        postprocess(data) {
    ...
    expect(Date.now() - start).toBeLessThan(baseline * 1.10);
});

it("Post-processing with promises", async () => {
    const loader = makeQueryLoader({
        query,
        db,
    ...
    expect(Date.now() - start).toBeLessThan(baseline * 1.15);
});
